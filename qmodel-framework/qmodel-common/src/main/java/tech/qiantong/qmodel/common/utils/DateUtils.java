...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils
{
    ...
    public static Date getLastWeekStartTime() {
        // 获取本周一（以周一为每周开始）
        LocalDateTime thisMonday = LocalDateTime.now()
                .with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY))
                .withHour(0).withMinute(0).withSecond(0).withNano(0);

        // 上周一 = 本周一 - 7天
        LocalDateTime lastMonday = thisMonday.minusDays(7);
        return Date.from(lastMonday.atZone(ZoneId.systemDefault()).toInstant());
    }
    ...
    public static Date getLastWeekEndTime() {
        ...
        // 上周日 = 本周一 - 1天
        LocalDateTime lastSunday = thisMonday.minusDays(1)
                .withHour(23).withMinute(59).withSecond(59).withNano(999_999_999);
        return Date.from(lastSunday.atZone(ZoneId.systemDefault()).toInstant());
    }

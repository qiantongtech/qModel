...
export function getValidateRule(item){
	...
	var checkConfig = (rule: any, value: any, callback: any) => {
		...
		if (itemProps.min) {
			let minDate = util.momentFunc(itemProps.min,"YYYY-MM-DD HH:mm:ss");
			let valueDate = util.momentFunc(value,"YYYY-MM-DD HH:mm:ss");
		...
		if (itemProps.max) {
			let maxDate = util.momentFunc(itemProps.max,"YYYY-MM-DD  HH:mm:ss");
			let valueDate = util.momentFunc(value,"YYYY-MM-DD  HH:mm:ss");

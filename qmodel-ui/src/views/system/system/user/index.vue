            ...
            >
              <template #default="{ node, data }">
                                <span class="custom-tree-node">
                                    <!-- 第一级 -->
                                    <el-icon
                                        class="colorwxz iconfont-mini icon-a-shuxingjiegouxianxing"
                                        v-if="node.expanded && node.level === 1"
                                    ><FolderOpened
                                    /></el-icon>
                                    <el-icon
                                        class="colorxz iconfont-mini icon-a-shuxingjiegouxianxing"
                                        v-if="!node.expanded && node.level === 1"
                                    ><Folder
                                    /></el-icon>
                                  <!-- 第二级 -->
                                    <el-icon
                                        class="iconfont-mini icon-a-shuxingjiegouxianxing colorwxz"
                                        v-if="
                                            node.expanded &&
                                    ...
                                    /></el-icon>
                                    <el-icon
                                        class="iconfont-mini icon-a-shuxingjiegouxianxing colorxz"
                                        v-if="
                                            !node.expanded &&
                                            node.childNodes.length &&
                                            node.level == 2
                                  ...
                                  <!-- 子级 -->
                                    <el-icon
                                        class="iconfont-mini icon-a-baogongxianxing colorwxz"
                                        v-show="!node.isCurrent && node.level == 3"
                                    ><Tickets
                                    /></el-icon>
                                    <el-icon
                                        class="iconfont-mini icon-a-baogongxianxing colorxz"
                                        v-show="node.isCurrent && node.level == 3"
                                    ><Tickets
                                    /></el-icon>
      ...
      <el-main>
        ...
        <div class="pagecont-bottom">
          ...
          >
            ...
            <el-table-column v-if="getColumnVisibility(0)" label="编号" align="center" key="userId" prop="userId"
                             width="80" sortable="custom" :sort-orders="['descending', 'ascending']"/>
            <el-table-column v-if="getColumnVisibility(1)" label="登录账号" key="userName" prop="userName"
                             :show-overflow-tooltip="{ effect: 'light' }"/>
            <el-table-column v-if="getColumnVisibility(2)" label="用户姓名" key="nickName" prop="nickName"
                             :show-overflow-tooltip="{ effect: 'light' }"/>
            <el-table-column v-if="getColumnVisibility(3)" label="部门" key="deptName" prop="dept.deptName"
                             :show-overflow-tooltip="{ effect: 'light' }"
            />
            <el-table-column v-if="getColumnVisibility(4)" label="手机号码" key="phonenumber" prop="phonenumber"
                             align="center" width="120"/>
            <el-table-column v-if="getColumnVisibility(5)" label="状态" align="center" key="status">
            ...
            <el-table-column v-if="getColumnVisibility(6)" label="创建时间" prop="createTime"
                             width="180" align="center" sortable="custom" :sort-orders="['descending', 'ascending']">
              <template #default="scope">
...
const getColumnVisibility = (key) => {
  ...
  if (!column) return true;// 如果没有找到对应列配置，默认显示
  return column.visible;// 如果找到对应列配置，根据visible属性来控制显示
};

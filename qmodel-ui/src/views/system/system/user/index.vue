      ...
      <el-form :model="detailForm" label-width="90px">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="用户性别">
              <div class="form-readonly">
                <dict-tag :options="sys_user_sex" :value="detailForm.sex" />
              </div>
            </el-form-item>
          </el-col>
        </el-row>
...
function departmentName(row) {
  return row.dept?.deptName || row.deptName;
}

/** 用户类型翻译 */
function userTypeFormat(row) {
  return row.userType === "00" ? "系统用户" : row.userType;
}
...
function handleView(row) {
  ...
  getUser(row.userId).then((response) => {
    ...
    const postIds = new Set((response.postIds || []).map(String));
    const roleIds = new Set((response.roleIds || []).map(String));

    detail.postGroupName = (response.posts || [])
      .filter((post) => postIds.has(String(post.postId)))
      .map((post) => post.postName)
      .join(", ");
    detail.roleGroupName = (response.roles || [])

...
<script setup>
    ...
    const loginLogoUpdate = (updatedFileList) => {
        ...
        loginLogoModelValue.value = import.meta.env.VITE_APP_BASE_API + '/profile' + updatedFileList;
    };

    const logoUpdate = (updatedFileList) => {
        contentDetail.value.logo = updatedFileList;
        logoModelValue.value = import.meta.env.VITE_APP_BASE_API + '/profile' + updatedFileList;
    };

    const carouselImageUpdate = (updatedFileList) => {
        const carouselImageList = updatedFileList.split(',');
        const arr = [];
        carouselImageList.forEach((e) => {
            if (e.indexOf(import.meta.env.VITE_APP_BASE_API) === -1) {
                arr.push({ url: import.meta.env.VITE_APP_BASE_API + '/profile' + e });
            } else {
                arr.push({ url: e });
            }
        });

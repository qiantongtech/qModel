...
<template>
  <div class="justify-between mb15">
    <el-row :gutter="15" class="btn-style">
      <el-col :span="1.5">
        <el-button type="primary" plain @click="handleAdd" v-hasPermi="['calcExecution:calcExecution:add']"
                   @mousedown="(e) => e.preventDefault()">
          <i class="iconfont-mini icon-xinzeng mr5"></i>新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain @click="handleExport" v-hasPermi="['calcExecution:calcExecution:export']"
                   @mousedown="(e) => e.preventDefault()">
  ...
  <el-table stripe height="374px" v-loading="loading" :data="calcExecutionList" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
    <el-table-column type="selection" width="55" align="center" />
            <el-table-column v-if="columns[0].visible" label="ID" align="center" prop="id" />
            <el-table-column v-if="columns[1].visible" label="计算任务ID" align="center" prop="calcId">
            ...
            <el-table-column v-if="columns[3].visible" label="执行批次号" align="center" prop="executionNo">
              <template #default="scope">
                    <dict-tag :options="${column.dictType}" :value="scope.row.executionNo"/>
              </template>
            </el-table-column>
            <el-table-column v-if="columns[4].visible" label="执行模式：1-手动触发 2-定时调度 3-重试执行" align="center" prop="executionMode">
              <template #default="scope">
                    <dict-tag :options="${column.dictType}" :value="scope.row.executionMode"/>
              </template>
            </el-table-column>
            <el-table-column v-if="columns[5].visible" label="执行状态：0-待执行 1-运行中 2-成功 3-失败 4-终止 5-排队中" align="center" prop="status">
              <template #default="scope">
                    <dict-tag :options="${column.dictType}" :value="scope.row.status"/>
              </template>
            </el-table-column>
            <el-table-column v-if="columns[6].visible" label="开始时间" align="center" prop="startTime" width="180">
              <template #default="scope">
                <span>{{ parseTime(scope.row.startTime, '{y}-{m}-{d}') }}</span>
              </template>
            </el-table-column>
            <el-table-column v-if="columns[7].visible" label="结束时间" align="center" prop="endTime" width="180">
              <template #default="scope">
                <span>{{ parseTime(scope.row.endTime, '{y}-{m}-{d}') }}</span>
              </template>
            </el-table-column>
            <el-table-column v-if="columns[8].visible" label="耗时(毫秒)" align="center" prop="duration">
              <template #default="scope">
                    <dict-tag :options="${column.dictType}" :value="scope.row.duration"/>
              </template>
            </el-table-column>
            <el-table-column v-if="columns[9].visible" label="输入参数快照(JSON，记录执行时的实际参数)" align="center" prop="inputParams">
              <template #default="scope">
                    <dict-tag :options="${column.dictType}" :value="scope.row.inputParams"/>
              </template>
            </el-table-column>
            <el-table-column v-if="columns[10].visible" label="输出结果(JSON)" align="center" prop="outputResult">
              <template #default="scope">
                    <dict-tag :options="${column.dictType}" :value="scope.row.outputResult"/>
              </template>
            </el-table-column>
            <el-table-column v-if="columns[11].visible" label="执行日志" align="center" prop="executionLog">
              <template #default="scope">
            ...
            <el-table-column v-if="columns[13].visible" label="当前重试次数" align="center" prop="retryCount">
              <template #default="scope">
                    <dict-tag :options="${column.dictType}" :value="scope.row.retryCount"/>
              </template>
            </el-table-column>
            <el-table-column v-if="columns[14].visible" label="资源使用情况(JSON，如CPU、内存、GPU使用)" align="center" prop="resourceUsage">
              <template #default="scope">
    ...
    <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="240">
      <template #default="scope">
        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                   v-hasPermi="['calcExecution:calcExecution:edit']">修改</el-button>
        <el-button link type="danger" icon="Delete" @click="handleDelete(scope.row)"
                   v-hasPermi="['calcExecution:calcExecution:remove']">删除</el-button>
        <el-button link type="primary" icon="view" @click="handleDetail(scope.row)"
                   v-hasPermi="['calcExecution:calcExecution:edit']">详情</el-button>
      </template>
    </el-table-column>
  ...
  <el-dialog :title="title" v-model="open" width="800px" :append-to="$refs['app-container']" draggable>
    ...
    <el-form ref="calcExecutionRef" :model="form" :rules="rules" label-width="80px">
                    <el-row :gutter="20">
                      <el-col :span="12">
...
<script setup name="ComponentOne">
  ...
  const { queryParams, form, calcExecutionDetail, rules } = toRefs(data);

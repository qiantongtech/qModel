...
<script setup>
    ...
    const rules = reactive({
        ...
        interfaceAddress: [{ required: true, message: '上传接口不能为空', trigger: 'blur' }],
        fileAddress: [{ required: true, message: '上传文件不能为空', trigger: 'blur' }]
    });
    ...
    const fileModelName = (res) => {
        console.log(res);
        form.fileName = res.originalFilename.substring(0, res.originalFilename.lastIndexOf('.'));
    };
    ...
    const handleDelete = (row) => {
        ...
        const status = row.status;
        if (status == 1) {
            ElMessageBox.confirm('【' + modelName + '】该版本已启用，请先停用后在删除！').catch(
                () => {}
            );
        } else {
            ElMessageBox.confirm('是否确认删除版本管理编号为"' + idsToDelete + '"的数据项？')
                .then(function () {
    ...
    const handleDownload = (row) => {
        ...
        downloadFile(modelForm.value.interfaceorfileAddress, modelForm.value.name + '模型文件');
    };

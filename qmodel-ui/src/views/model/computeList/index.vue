            ...
            <el-form ref="formref" :model="form" :rules="rules" label-width="95px">
                ...
                <el-form-item label="模型名称" prop="modelId">
                    <el-select
                        ...
                        placeholder="请选择模型名称"
                        clearable
                        @change="(getAllModelVersion(form.modelId), (form.modelVersion = null))"
                        value-key="id"
                    >
                        <el-option
                ...
                <el-form-item label="模型版本" prop="modelVersion">
                    <el-select v-model="form.modelVersion" placeholder="请选择模型版本" clearable>
                        <el-option
...
<script setup>
        ...
        return new Promise((resolve) => {
            findModelInputById(param).then((res) => {
                ...
                for (let i = 0; i < inputList.value.length; i++) {
                    inputList.value[i].singleContent = JSON.parse(inputList.value[i].singleContent);
    ...
    const submitFormParams = () => {
        ...
        let endDate = moment(inputJson.value[name][inputJson.value[name].length - 1][0]).format(
            'YYYY-MM-DD'
        );
    ...
    const handleDelete = (row) => {
        const calcIds = row.id || ids.value;
        ElMessageBox.confirm('是否确认删除模型计算管理 编号为"' + calcIds + '"的数据项？')
            .then(function () {

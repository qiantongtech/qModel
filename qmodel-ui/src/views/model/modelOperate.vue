...
<script setup>
    ...
    const typeFormat = (dict, row) => {
        return proxy.selectDictLabel(proxy.dict.type[dict], row);
    };
    ...
    const handleDelete = (row) => {
        ...
        ElMessageBox.confirm(`是否确认删除模型历史管理 编号为"${idsToDelete}"的数据项？`, '警告', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        })
            .then(() => {
                return delOperate(idsToDelete);
            })
            .then(() => {
                getList();

        ...
        <el-row :gutter="0" style="margin-bottom: 10px">
            ...
            />
                    ...
                    <el-descriptions-item label="创建人" span="1">
                        <span>{{ viewInfo.createBy != null ? viewInfo.createBy : '--' }}</span>
                    </el-descriptions-item>
                    <el-descriptions-item label="上传时间" span="1">
                        <span>{{ viewInfo.createTime != null ? viewInfo.createTime : '--' }}</span>
                    </el-descriptions-item>
                ...
                <el-row>
                    <el-col :span="20">
                        <el-form-item label="模型名称：" prop="name">
                            <el-input clearable v-model="form.name" placeholder="请输入模型名称" />
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
...
<script setup>
    ...
    const rules = reactive({
        ...
        classifyId: [{ required: true, message: '模型分类不能为空', trigger: 'blur' }],
        name: [{ required: true, message: '模型名称不能为空', trigger: 'blur' }],
        version: [{ required: true, message: '版本号不能为空', trigger: 'blur' }],
        description: [{ required: true, message: '版本发布说明不能为空', trigger: 'blur' }],
        interfaceAddress: [{ required: true, message: '模型调用接口不能为空', trigger: 'blur' }]
    });
    ...
    const isWhetherPublish = () => {
        ...
        if (whetherPublish == 1) {
            ...
            viewInfo.value.whetherPublish = whetherPublish;
            updateModel(viewInfo.value).then((response) => {
                ElMessage.success(viewInfo.value.whetherPublish == 0 ? '以取消发布' : '发布成功');
                getList();
            });
        }
    };

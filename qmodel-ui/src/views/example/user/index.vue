    ...
    <div  class="pagecont-bottom">
      <div class="justify-between mb15">
        <el-row :gutter="15" class="btn-style">
          <el-col :span="1.5">
            <el-button type="primary" plain @click="handleAdd" v-hasPermi="['user:userType:add']"
                       @mousedown="(e) => e.preventDefault()">
              <i class="iconfont-mini icon-xinzeng mr5"></i>新增
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="primary" plain :disabled="single" @click="handleUpdate" v-hasPermi="['user:userType:edit']"
                       @mousedown="(e) => e.preventDefault()">
          ...
          <el-col :span="1.5">
            <el-button type="warning" plain :disabled="multiple" @click="handleExport" v-hasPermi="['user:userType:export']"
                       @mousedown="(e) => e.preventDefault()">
      ...
      <el-table stripe height="498px" v-loading="loading" :data="userTypeList" @selection-change="handleSelectionChange" :default-sort="{ prop: 'createTime', order: 'descending' }">
        <el-table-column type="selection" width="55" align="center" />
        <el-table-column v-if="columns[0].visible" label="ID" align="center" prop="id" />
        <el-table-column v-if="columns[1].visible" label="类型名称" align="center" prop="name">
          <template #default="scope">
            {{ scope.row.name || '-' }}
          </template>
        </el-table-column>
        <el-table-column v-if="columns[2].visible" label="是否有效" align="center" prop="validFlag">
          <template #default="scope">
            {{ scope.row.validFlag || '-' }}
          </template>
        </el-table-column>
        <el-table-column v-if="columns[3].visible" label="删除标志" align="center" prop="delFlag">
          <template #default="scope">
            {{ scope.row.delFlag || '-' }}
          </template>
        </el-table-column>
        <el-table-column v-if="columns[4].visible" label="创建人" align="center" prop="createBy">
          <template #default="scope">
            {{ scope.row.createBy || '-' }}
          </template>
        </el-table-column>
        <el-table-column v-if="columns[5].visible" label="创建人id" align="center" prop="creatorId">
          <template #default="scope">
            {{ scope.row.creatorId || '-' }}
          </template>
        </el-table-column>
        <el-table-column v-if="columns[6].visible" label="创建时间" align="center" prop="createTime" width="180" sortable>
          <template #default="scope">
            <span>{{ parseTime(scope.row.createTime, '{y}-{m}-{d}') }}</span>
          </template>
        </el-table-column>
        <el-table-column v-if="columns[7].visible" label="更新人" align="center" prop="updateBy">
          <template #default="scope">
            {{ scope.row.updateBy || '-' }}
          </template>
        </el-table-column>
        <el-table-column v-if="columns[8].visible" label="更新人id" align="center" prop="updatorId">
          <template #default="scope">
            {{ scope.row.updatorId || '-' }}
          </template>
        </el-table-column>
        <el-table-column v-if="columns[9].visible" label="更新时间" align="center" prop="updateTime" width="180" sortable>
          <template #default="scope">
...
import { listUserType, getUserType, delUserType, addUserType, updateUserType } from "@/api/example/user/userType";
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除用户类型编号为"' + _ids + '"的数据项？').then(function() {
...
function handleExport() {
  proxy.download('example/userType/export', {
    ...queryParams.value

  ...
  <div class="weather-wrap">
    ...
    <div class="weather-main">
      <div class="weather-current">
        <img
          class="current-icon"
          :src="getWeatherIcon(weather.now?.text)"
          alt=""
        />
    ...
    <div class="weather-forecast">
      ...
      >
        ...
        <span
          v-if="index < forecastList.length - 1"
...
function getWeatherIcon(name) {
  const iconName =
    iconNameMap[name] || (knownIconNames.includes(name) ? name : "晴");
  return new URL(
    `../../assets/system/images/index/weather/${iconName}.png`,
    import.meta.url
  ).href;
}
...
async function getData() {
  ...
  const baseParams =
    "key=SjyiLD_odjCGOsHoF&location=南京&language=zh-Hans&unit=c";
  ...
  try {
    ...
    weather.daily = dailyResult?.daily?.length
      ? dailyResult.daily
      : fallbackWeather.daily;
  } catch {

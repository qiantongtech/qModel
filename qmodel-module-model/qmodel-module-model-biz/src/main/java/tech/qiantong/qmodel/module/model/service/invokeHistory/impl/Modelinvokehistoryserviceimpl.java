...
public class Modelinvokehistoryserviceimpl extends ServiceImpl<ModelInvokeHistoryMapper, ModelInvokeHistoryDO> implements IModelInvokeHistoryService {
    ...
    public void saveInvokeLogAsync(Long modelId, String modelName, String invokeType, String inputParams,
        ...
        try {
            ...
            invokeHistory.setStatus(status);
            invokeHistory.setErrorMessage(errorMessage);
            invokeHistory.setDuration(duration != null ? duration : 0L);
            invokeHistory.setStartTime(startTime);
            invokeHistory.setEndTime(endTime);
            invokeHistory.setClientIp(clientIp);
            invokeHistory.setCreateBy("system");
            invokeHistory.setCreatorId(1L);
            invokeHistory.setCreateTime(new Date());
            invokeHistory.setUpdateBy("system");
            invokeHistory.setUpdatorId(1L);
            invokeHistory.setUpdateTime(new Date());
            invokeHistory.setValidFlag(true);
            invokeHistory.setDelFlag(false);

            modelInvokeHistoryMapper.insertInvokeHistoryWithoutFill(invokeHistory);
        } catch (Exception e) {
            log.error("保存模型调用历史记录失败，错误信息：" + e.getMessage(), e);
        }
    }
}
